  ...
  (e) => {
    ...
    var logSync = (status, rowsRead, rowsSaved, errorLog) => {
      ...
      try {
        ...
        $app.logger().error('sync_pull_sheets: erro ao registrar historico: ' + String(err))
      }
    }
    ...
    try {
      ...
      $app.logger().error('sync_pull_sheets: falha na requisicao HTTP: ' + String(err))
    ...
    for (var i = 1; i < values.length; i++) {
      ...
      if (!exiId) {
        ...
        $app
          .logger()
          .info(
            'sync_pull_sheets: pulado: sem ID_SERVICO - ' +
      ...
      try {
        ...
        $app
          .logger()
          .error('sync_pull_sheets: erro ao processar linha ' + i + ': ' + String(recordErr))
      }
    }
    ...
    for (var mesKey in monthStats) {
      ...
      $app
        .logger()
        .info(
          'sync_pull_sheets: resumo mensal - mes: ' +
            mesKey +
            ' | linhas lidas: ' +
            s.lidas +
            ' | criados: ' +
    ...
    $app
      .logger()
      .info(
        'sync_pull_sheets: sincronizacao concluida - criados: ' +
          created +
          ', atualizados: ' +
          updated +
          ', ignorados: ' +
