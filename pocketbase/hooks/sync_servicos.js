cronAdd('sync_servicos', '0 * * * *', () => {
  ...
  try {
    ...
    $app.logger().error('sync_servicos: HTTP request failed', 'error', String(err))
    return
  }
  ...
  if (res.statusCode !== 200) {
    $app
      .logger()
      .error('sync_servicos: non-200 response from Google Sheets', 'statusCode', res.statusCode)
    return
  }
  ...
  try {
    ...
    $app.logger().error('sync_servicos: failed to decode response body', 'error', String(err))
    return
  }
  ...
  if (rows.length < 2) {
    $app.logger().warn('sync_servicos: CSV has no data rows, skipping sync')
    return
  }
  ...
  try {
    ...
    $app.logger().error('sync_servicos: servicos collection not found', 'error', String(err))
    return
  }
  ...
  try {
    $app.runInTransaction((txApp) => {
      ...
      $app.logger().info('sync_servicos: sync completed', 'recordsInserted', inserted)
    })
  } catch (err) {
    $app
      .logger()
      .error('sync_servicos: transaction failed, previous data retained', 'error', String(err))
  }
})
